...
class TestConcurrentReadWriteConsistencyPass {
	...
	void test() throws RemoteException, InterruptedException {
		...
		// GOAL: A write/read operation must not leave the distributed data store in an inconsistent state
		...
		client2 pr2 = new client2("process2", p2); 			// process2 receives read request from client2
		...
		Assertions.assertFalse(result); 								// This must assert to false (pass) i.e. pr1 and pr2 = false (at least one must fail to avoid read/write conflicts)
	...
	class client1 extends Thread {
		...
		public void run() {
			try {
				Message wmessage = buildWriteMessage(process);  						// build a message for one process
				
				boolean decision = recprocess.requestWriteOperation(wmessage);			// send a write request and get the voters decision				
	...
	class client2 extends Thread {
		...
		public void run() {
			try {
				Message rmessage = buildReadMessage(process);
				
				// a read request comes to a particular process
				boolean decision = recprocess.requestReadOperation(rmessage); 					// get the voters decision						
				rmessage.setAcknowledged(decision); 											// put the voters' decision in a message
				recprocess.multicastVotersDecision(rmessage); 									// multicast the decision to others
